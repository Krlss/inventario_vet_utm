                    ...
                    <tr>
                        ...
                        <td>
                            <input type="number" min='1' name="products[{{ $index }}][quantity]"
                                class="form-control" wire:model="products.{{ $index }}.quantity" />
                            @if ($errors->has('products.' . $index . '.quantity'))
                                <span
                                    class="text-red-500 text-xs italic">{{ $errors->first('products.' . $index . '.quantity') }}</span>
                            @endif
                        </td>
                        <td>
                            @if ($type == 'ingress')
                        ...
                        @if ($expire)
                            <td>
                                <input type="date" name="products[{{ $index }}][expire]" class="form-control"
                                    wire:model="products.{{ $index }}.expire" />
                                @if ($errors->has('products.' . $index . '.expire'))
                                    <span
                                        class="text-red-500 text-xs italic">{{ $errors->first('products.' . $index . '.expire') }}</span>
                                @endif
                            </td>
                        @endif
                        <td>

    ...
    <div class="card-body pt-0 mt-0">
        ...
        <div class="flex md:flex-row flex-col justify-between md:items-end items-start gap-2 mb-2">
            ...
            <div class="flex xs:items-end items-start justify-between w-full xs:flex-row flex-col gap-2">
                ...
                @can('inventory.ingress-products.create')
                <a href="{{ route('dashboard.products-ingress.create') }}" class="bg-green-page text-white py-2 px-4 hover:bg-green-900 rounded-md font-medium hover:no-underline">
                    {{__('New Income')}}
                </a>
                @endcan
            </div>
...
@push('js')
@include('dashboard.products-ingress.scripts')
@endpush
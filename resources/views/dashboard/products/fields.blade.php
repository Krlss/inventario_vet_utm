<div class="flex flex-col gap-2">
    <div class="grid md:grid-cols-3 grid-cols-1 gap-2">
        ...
        <div>
            ...
            <select class="form-control" id="id_unit" name="id_unit" required>
                ...
                <option value="{{ $unit->id }}" {{ old('id_unit') == $unit->id ? 'selected' : '' }}>{{ $unit->name }}</option>
                @endforeach
            </select>
    ...
    <div class="grid md:grid-cols-3 grid-cols-1 gap-2">
        <div>
            ...
            <input min="0" pattern="[0-9]+" type="number" class="form-control" id="amount" name="amount" placeholder="{{ __('Amount') }}" value="{{old('amount') ?? 0}}" required>
        ...
        <div>
            ...
            <input min="0" pattern="[0-9]+" type="number" class="form-control" id="stock_min" name="stock_min" placeholder="{{ __('Stock min') }}" value="{{old('stock_min') ?? 0}}" required>
    ...
    <div class="grid grid-cols-1 gap-2">
        <div>
            ...
            <select class="form-control" id="id_type" name="id_type[]" multiple>
                ...
                <option value="{{ $type->id }}" {{ (collect(old('id_type'))->contains($type->id)) ? 'selected':'' }}>{{ $type->name }}</option>
                @endforeach
            </select>
    ...
    <div class="grid grid-cols-1 gap-2">
        <div>
            ...
            <select class="form-control" id="id_category" name="id_category[]" multiple>
                ...
                <option value="{{ $category->id }}" {{ (collect(old('id_category'))->contains($category->id)) ? 'selected':'' }}>{{ $category->name }}</option>
                @endforeach
            </select>

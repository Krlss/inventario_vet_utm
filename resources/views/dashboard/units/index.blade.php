...
<div class="card">
    <div class="card-body pt-0 mt-0">
        ...
        <div class="mt-2 mb-4 flex md:flex-row flex-col md:items-center items-start justify-between gap-2">
            <span class="font-bold text-lg">{{__('Units registered in the system')}}</span>
            @can('inventory.units.create')
            <x-button-add-modal target="unit" />
            @endcan
        </div>
...
@push('js')
@include('dashboard.units.datatable')
...
<script>
    $('#add_data').click(function() {
        ...
        $('#modal_class').val('units'); // This is the class that will be used in the model 
        $('div.flash-message').html('');
        $('.modal-title').text("{{__('Creating a new unit')}}");
    });
    $('#universalModal').on('show.bs.modal', function(event) {
        ...
        $('.modal-title').text("{{__('Editing unit')}}");
